/**
 * 1학년 1학기 국어·수학 areas + 평어 문장 DB 시드
 *
 * 순서:
 * 1) subject가 '국어' 또는 '수학'이고 semester=1인 areas 삭제 (CASCADE로 templates/ratings 함께 삭제)
 * 2) areas 파일 2개 읽어서 INSERT, 생성된 id 저장
 * 3) 평어 문장 파일 2개 읽어서 areaName으로 area_id 매핑 후 templates INSERT
 *
 * 사용 (next-app 폴더에서):
 *   node scripts/seed-국어수학-평어.mjs
 */
...
async function main() {
  ...
  console.log('areas: 국어·수학 semester=1 삭제 완료');

  // 2) areas 파일 2개 INSERT, (subject, name) → id 저장
  const areaFiles = [
    { file: '국어-1학년1학기-areas.json', subject: '국어' },
    { file: '수학-1학년1학기-areas.json', subject: '수학' },
  ];
